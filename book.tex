...
A \emph{neural network} (\textbf{NN})~\cite{Goodfellow-et-al-2016} consists of an input layer, multiple hidden layers, and an output layer. Each layer has a number of neurons, each connected to neurons in the next layer through a predefined set of weights (computed during training). A \emph{Deep Neural Network} (\textbf{DNN}) is an NN with \emph{two} or more hidden layers.
...
Use Z3 to encode the DNN in~\autoref{fig:dnn} and compute its output $x_5$ for the input $(x_1, x_2) = (2.0, 0.5)$.
...
\begin{itemize}
    ...
    \end{itemize}
\item Come UB with \textbf{three} properties that this network \emph{does not} have.  Recall that a property often has the form in~\autoref{sec:properties-def}.
\begin{itemize}
    \item Argue why each property does not hold by providing a counterexample (\autoref{sec:properties-counterexamples}).
    \item (Optional, Easy) Try to use Z3 to show that the properties you came UB with do not hold by asking Z3 to find counterexamples (one for each property).
\end{itemize}
\item Come UB with \textbf{three} properties that that this network has (i.e., as long as the precondition holds, the postcondition holds).
\begin{itemize}
    ...
    \item (Optional) Show that the properties you came UB will always hold by using Z3 to prove that no counterexample exists (i.e., \unsat{}).
\end{itemize}
\end{itemize}
...
\begin{itemize}
    ...
    \item Generate the satisfiability formula (\autoref{eq:nnv2}).   You do not need to use Z3 to check it.
\end{itemize}
\end{problem}
...
\begin{itemize}
    ...
    Moreover, MILP solvers, even industrial-strength ones such as Gurobi~\cite{gurobi}, are primarily CPU-based and does not leverage the massive parallelism provided by modern GPUs to speed UB computation.
...
% A \emph{zonotope} captures this correlation by encoding both variables as linear combinations of a few shared ``uncertainty sources’’ (\(\epsilon_i\)’s).
...
%   % corners: c ± g1 ± g2  (center at origin)
...
has two \emph{activation statuses}: active (identity function) and inactive (zero function). Each status
partitions the input space into two regions---active and inactive.
...
Once having an activation pattern $p$, we can simplify the formula $\alpha$ representing the DNN by replacing each ReLU function with a linear constraint according to the activation status specified in $p$. For example, if $p$ specifies that neuron $n_i$ is active, we replace $\relu{z_i}$ with the constraint $z_i \ge 0$, i.e., the pre-activation value $z_i$ must be non-negative. Otherwise, if $n_i$ is inactive, we replace $\relu{z_i}$ with $z_i < 0$.
...
Do~\autoref{ex:bab} but come UB with concrete values for each step, e.g., what are the bounds computed by \Deduce, what does \mycode{LP} return, etc to illustrate the process in more detail.
\end{problem}
...
As DNN tools become more complex (e.g., state-of-the-art tools have 20K+ LoCs), they are more prone to bugs. VNN-COMP'23~\cite{brix2023fourth} showed that 3 of the top 7 participants produced unsound results by claiming unsafe DNNs are safe, i.e., they produce \unsat{} on problems that are actually \sat{}.
...
\autoref{alg:bab-proof} extends~\autoref{alg:bab} to show \proofgen, a BaB-based DNN verification algorithm with proof generation capability. The key idea is to introduce a \textbf{proof tree} (\autoref{line:prooftree}) and recording the branching decisions to it (\autoref{line:record_proof}).
...
We reuse the example in~\autoref{ex:bab} to illustrate \proofgen{}. Recall the goal is to verify that the DNN in~\autoref{fig:proof-example}(a) has the property $(x_1, x_2) \in [-2.0, 2.0] \times [-1.0, 1,0] \Rightarrow (y_1 > y_2)$.
...
\begin{problem}\label{prob:proof-tree}
...
We present \proofcheck{}, a proof checker for \prooflang{} proofs generated by BaB-based DNN verifiers.
...
with the constraints in~\autoref{eq:nnv2} representing the input ranges and the DNN with the objective of optimizing the output. \proofcheck{} then invokes an LP solver, which determines that this MILP is infeasible, i.e., leaf node 3 indeed leads to unsatisfiability.
...
% We can keep backing UB until it cannot prove anymore, however, after N times, each time half the length of an activation pattern, an activation pattern is shortened to $1 / 2^{N}$ (e.g., if $N=2$, after 2 times of backing up, an activation pattern is shortened to 1/4 of its length).
...
% While the vast majority of the verification problems have proof trees of fewer then 2000 leaves, but 17 of them have larger trees UB to a maximum of more than 10000 leaves.
...
Finally, because each task can be solved independently, the verifier can solve them in parallel to further speed UB the verification process.
...
The verifier then checks if the DNN produces the desired output from each of these subregions.
...
\begin{example}\label{ex:inputboundstigthten}
Recall the DNN from~\autoref{fig:dnn-b} can be represented as:
\[
\begin{aligned}
& \hat{x}_3 = -0.5x_1 + 0.5x_2 + 1.0 ~\land\\
& \hat{x}_4 = 0.5x_1 - 0.5x_2 + 1.0 ~\land\\
...
Note that, this approach works well for networks with small inputs, e.g., the ACAS Xu benchmark~\autoref{sec:acasxu} with 5 inputs, and is adopted by many modern DNN verification tools, including \marabou{} and \nnenum{}.
...
To reduce the time complexity, we can prioritize the input variables to tighten first, e.g., deciding the variable with the largest bounds first, and only tighten UB to a maxinum number of inputs, e.g., 10 inputs.
...
\begin{example}\label{ex:neuronboundstigthten}
...
\begin{example}\label{ex:matrix-form-equivalence}
...
\begin{example}\label{ex:batch-processing}
...
By using matrix operations, we can leverage using GPUs natively to speed UB the computation.
...
\begin{figure}[t]
    ...
    \caption{GPU Benchmarking Results.}\label{fig:gpu-benchmarking}
\end{figure}
...
\neuralsat{}~\cite{duong2025neuralsat,duong2024harnessing} is a relatively new competitor in the DNN verification space, but it has quickly become a strong contender, consistently placed among the top tools at DNN verification competitions~\autoref{chap:vnncomps}.
...
This representation enables using standard DPLL to search for truth values satisfying these clauses and a DNN-specific theory solver to check the feasibility of truth assignments---\emph{activation patterns} (\autoref{sec:activation-patterns})--- with respect to the constraints encoding the DNN and the property of interest.
...
\section{Algorithm Overview}\label{sec:reluplex-overview}


\reluplex{} extends the classical simplex method so that it can handle the
...
\begin{figure}
    \centering
    \mydnnthree{1}
    \caption{\label{fig:dnn-replux}A simple DNN.}
\end{figure}
...
\begin{example}\label{ex:reluplex}
  ...
  We demonstrate \reluplex{} using the DNN in~\autoref{fig:dnn-replux}. This network can be represented by the following equations:
...
We start by introducing three \emph{basic} (auxilliary or slack) variables to represent the relationships between the variables in the DNN:
...
Thus, the property in~\autoref{eq:prop1} is \emph{not valid} for the DNN in~\autoref{fig:dnn-replux} because for inputs $x_1=0,x_2=-1$, the network gives the output $x_5=0.5$, violating the property.
\end{example}


\begin{problem}\label{problem:reluplex-hand}
    ...
    \item Provide feedback on this example.  Is it clear?  Are there any steps or terminologies that are confusing?  How can we improve the explanation?  For this part you can type directly on a text file and submit.
    \end{itemize}
\end{problem}
...
\begin{problem}\label{problem:reluplex-basic-nonbasic}
...
% \paragraph*{Motivation} This benchmark tries to scale UB the size of networks being analyzed by using the well-studied VGGNET-16 architecture~\cite{simonyan2014very} that runs on ImageNet. Input-output properties are proposed on pixel-level perturbations that can lead to image misclassification.
...
As the NNV field matured, it became increasingly difficult to compare approaches. Papers often evaluated tools on different benchmarks, under different hardware assumptions, and using different specification languages. This fragmentation made it hard to assess the true state of the art, identify open challenges, and drive progress.

Inspired by the success of other formal methods competitions (e.g., SAT, SMT, model checking), the International Verification of Neural Networks Competition (\textbf{VNN-COMP}) was created to address these issues by providing a standardized, reproducible, an automated evaluation framework for neural network verification tools. VNN-COMP has been held annually since 2020, with each edition refining rules, benchmarks, and infrastructure based on community feedback and lessons learned.



\section{The 2025 Edition at a Glance}
...
\section{Organization and Timeline}
...
\section{Perspective: Why VNN-COMP Matters}
...
\chapter{Formal Methods}\label{chap:formal-methods}

Neural network verification (NNV) is a rising topic that applies \emph{formal methods} (FM) to machine learning systems, particularly neural networks (NN).  This chapter explains what FM are, how verification differs from testing and debugging, and key concepts such as specifications, soundness, and completeness. It provides the foundational knowledge about formal methods necessary to understand NNV.

\section{What Are Formal Methods (FM)?}

FM are techniques for analyzing systems, e.g., computer software, using \emph{mathematically precise models and reasoning}. In FM the behavior of a program is described and analyzed using logic and mathematics rather than informal reasoning or empirical testing. This allows formal methods to provide \emph{guarantees} about system properties. For example, an FM might prove that a sorting algorithm always produces a correctly ordered list for any input list

To motivate the need for FM, consider a simple function that computes the absolute value of an integer, where the goal is to show that the function always returns a non-negative number.
...
\begin{equation}\label{eq:abs_val_nonneg}
...
\section{Specifications}\label{sec:fm-spec}
...
\section{FM Technique}
% ## Formal Methods as Tools: Models, Specifications, and Reasoning

At a high level, an FM technique is an algorithm and implemented as a tool (a program) that analyzes a system.  The FM tool takes as inputs
\begin{itemize}
    \item A \emph{specification} of the desired property of the system
    \item A \emph{model} of the system to be analyzed
\end{itemize}
and applies a \emph{reasoning process} to determine whether the model satisfies the specification. The tool produces an \emph{outcome} indicating whether the property holds, does not hold, or is unknown.
...
Once the model and specification are given, a FM tool applies a \emph{reasoning process}---an algorithm---to determine whether the model satisfies the specification. 
...
\item Unknown: the tool cannot determine whether the model satisfies the specification.
\end{enumerate}
...
\section{Major Classes of Formal Methods Algorithms}

% *(Using One Running Example)*

We now introduce the major classes of formal-methods techniques. All of these techniques \emph{statically analyze}--- analyzing the program representation (e.g., code) without running the program---the model and specification to determine whether the property holds.

We will use the `clip` function in~\autoref{ex:clip} as a running example demonstrate how each technique reasons about the same model and specification.



\paragraph{Model Checking}

Model checking (MC) works by \emph{exploring all possible states} of a system and checking whether \emph{bad states} which violate the specification are reachable. MC works well for systems with a finite number of states such as hardware designs and communication protocols because these systems have finite state spaces that can be exhaustively explored. However, MC struggles with systems involving continuous variables or infinite state spaces, e.g., a program with loops. 
...
A standard (naive) MC would struggle here because the number of states is infinite (the input $x$ is a real number). So MC would need to discretize or bound the input space, e.g., by considering only integer values of x from -1000 to 1000, to make the analysis feasible.
\end{example}

\paragraph{Interactive Theorem Proving}  
An interactive theorem prover (ITP) or \emph{proof assistant} attempts to reason about the program using \emph{logical inference rules} to construct a formal proof that the specification holds for all possible inputs. The reason it is called ``interactive'' is that the user must guide the proof process by by providing lemmas, definitions, and strategies to help the prover construct the proof.  ITPs are powerful and can prove deep rich properties about programs. However, they may require significant human guidance to construct the proofs and do not scale well to large or complex systems. 
...
Abstract interpretation (AbsInt) analyzes a program by automatically computing \emph{over-approximations} of its behavior. Instead of tracking exact values, AbsInt tracks abstract properties such as ranges or signs of variables, e.g., instead of saying ``x = 5'', AbsInt might say ``x is in [0, 10]'' or ``x is non-negative''. Because AbsInt is fully automated and works with over-approximations, it is efficient and scales well to large programs. However, this comes at the cost of precision. AbsInt may produce false positives (reporting potential violations that are not real bugs) due to the over-approximation.
...
% ## Why This Matters for Neural Network Verification

% Neural network verification tools follow the **same structure**:

% * The neural network is the **model**
% * Robustness or safety conditions are the **specification**
% * SMT, abstract interpretation, or hybrid techniques perform the **reasoning**
% * The result is **proved, disproved, or unknown**

% Nothing about this framework is unique to neural networks. What is new is the complexity of the model.

% Understanding formal methods at this level is essential before studying neural network verification algorithms.
...
% Two core concepts in formal methods are **soundness** and **completeness**. These describe the reliability of a verification technique.

% A verification method is **sound** if every property it claims to prove is actually true. In other words, a sound verifier never lies. If it says “this property holds,” then the property truly holds.

% A verification method is **complete** if it can prove every true property. That is, if the property holds, the verifier will always succeed in proving it.

% In practice, most verification tools cannot be both sound and complete for complex systems. Neural network verification problems are computationally hard, involving nonlinear functions and high-dimensional spaces. As a result, many NNV tools are designed to be sound but incomplete. They may fail to prove some true properties, but they will never incorrectly prove a false one.

% This tradeoff is intentional. In safety-critical contexts, a false proof is unacceptable. An inconclusive result is inconvenient but safe.

% ---

% ## 5. Formal Methods Techniques in Practice

% Formal methods encompass several families of techniques, each suited to different kinds of systems.

% **Model checking** explores a system’s state space to determine whether undesirable states are reachable. It has been extremely successful in hardware verification and communication protocols but struggles with systems that involve continuous values.

% **Theorem proving** uses logical inference to prove correctness properties. It is highly expressive but often requires significant human guidance.

% **SAT and SMT solving** reduce verification problems to questions of logical satisfiability. Modern SAT and SMT solvers are highly optimized and form the backbone of many verification tools. Neural network verification heavily relies on these solvers to reason about constraints induced by network layers and activation functions.

% **Abstract interpretation** analyzes systems by computing over-approximations of their behavior. It sacrifices precision in exchange for scalability while maintaining soundness.

% Neural network verification combines ideas from SMT solving, optimization, and abstract interpretation to handle the numerical and nonlinear nature of neural networks.

% ---

% ## 6. Neural Network Verification as Formal Verification

% Neural networks are often viewed as opaque or mysterious, but from a formal methods perspective, they are deterministic mathematical functions. Once trained, a neural network computes a fixed function from inputs to outputs.

% This makes neural networks amenable to formal reasoning. A verification problem for a neural network typically asks whether the function computed by the network satisfies a given specification for all inputs in a specified region.

% If the property holds, the verifier produces a proof. If it does not hold, the verifier produces a **counterexample**—a specific input that violates the property. This counterexample is not a heuristic or guess; it is a mathematically valid witness showing that the property is false.

% This structure mirrors classical formal verification exactly. Neural network verification is not a new paradigm—it is a new application domain for established formal methods ideas.

% ---

% ## 7. Why Formal Methods Matter for Machine Learning

% Machine learning systems are increasingly deployed in real-world settings where failures can cause harm. In such settings, average-case performance metrics like accuracy are insufficient. Rare failures matter precisely because they are rare and unexpected.

% Testing can reveal many problems, but it cannot rule out all possible failures. Formal methods provide a way to reason about worst-case behavior and to establish guarantees that testing cannot.

% Neural network verification exists because learned models are now used in contexts where guarantees matter. It provides tools for answering questions that testing alone cannot answer.

% ---

% ## 8. Limits of Formal Methods

% Formal methods do not solve all problems. Verification can only reason about the model and specification provided. If the specification is incomplete or unrealistic, the guarantees are limited.

% Verification does not ensure that a model is well-trained, unbiased, or appropriate for its task. It ensures only that the model satisfies the specified properties.

% Understanding these limits is part of understanding formal methods. They provide precise answers to precise questions—not vague assurances.

% ---

% ## 9. Summary

% Formal methods provide mathematically rigorous techniques for reasoning about system behavior. Verification differs fundamentally from testing and debugging by offering guarantees rather than empirical confidence. Neural network verification is a direct extension of these ideas to machine-learned models.

% With this background, the reader is now prepared to study the concrete neural network verification techniques presented in the rest of this book.
...
This part provides the necessary background on both topics in detail, starting with propositional logic and satisfiability, and then moving to linear and mixed-integer programming.
...
An \emph{assignment} $\sigma$ maps each formula to $\texttt{True}$ or $\texttt{False}$.
...
\begin{align*}
  ...
  \sigma(\phi \to \psi) = \texttt{True} &\quad \text{iff } \sigma(\phi) = \texttt{False} \text{ or } \sigma(\psi) = \texttt{True}.
\end{align*}
...
\begin{itemize}
  \item is \emph{satisfiable} if there exists \textbf{some} $\sigma$ such that $\sigma(\phi) = \texttt{True}$.
  \item is \emph{unsatisfiable} if \textbf{no} assignment satisfies it.
  \item is \emph{valid} if \textbf{all} assignments satisfy it.
\end{itemize}
\end{definition}

\begin{example} The formula $x$ is satisfiable (e.g., $\sigma(x) = \texttt{True}$).
...
\subsection{SAT Checking}\label{sec:sat-solving}

\emph{A SAT solver} takes a propositional formula, typically in CNF, as input and determines its satisfiability by searching for a satisfying assignment $\sigma$ that makes the formula true. If one exists, it returns \sat{} (and optionally $\sigma$); otherwise, it reports \unsat{}.
...
\subsection{Complexity of SAT}\label{sec:complexity-sat}
...
\section{Satisfiability Modulo Theories (SMT)}\label{chap:smt}
...
\end{example}
\subsection{SMT Solvers}\label{sec:smt-solvers}
...
\section{Z3 SMT Solver}\label{sec:z3}
...
\chapter{SAT Solving Algorithms}\label{chap:sat-solving-algorithms}
...
% \caption{Implication chain created by BCP.}\label{fig:bcp-chain}
% \end{tikzpicture}
% \end{figure}


\subsection{Conflict Analysis and Backtracking}\label{sec:conflict-analysis}
...
% \end{tabular}
% \end{table}
% \chapter{Comparing neural networks with software}\label{chap:nn-vs-software}
...
\section{DPLL}\label{sec:ns-dpll}
...
%\hd{Other backtracking scheme in Chaff~\cite{moskewicz2001chaff}: always stop clause learning at the first UIP --- This form of clause learning, and associated backtracking procedure, is referred to as first UIP clause learning. Existing results indicate that the first UIP clause learning procedure may end UB doing more backtracking than the original clause learning of GRASP~\cite{zhang2001efficient}. However, Chaff creates significantly fewer clauses and is significantly more effective at backtracking. As a result, the first UIP learning scheme is now commonly used by the majority of CDCL SAT solvers.}
...
At every step of the search algorithm, we select UB to $n$ nodes  of the DPLL(T) search tree to create a beam of width $n$.
...
\part{Programming Assignments and Schedule\label{part:pas-schedule}}
...
\begin{itemize}
    \item Week 1 (Aug 27): 
    \begin{itemize}
        \item Introduction (basic terminologies and concepts).
        \item Videos of neural networks 
        \begin{itemize}
    ...
    \item Topics: Background on Logic~\autoref{chap:logic} and Linear Programming~\autoref{chap:lp}
