...
Use Z3 to encode the DNN in~\autoref{fig:dnn} and compute its output $x_5$ for the input $(x_1, x_2) = (2.0, 0.5)$ and find an input that maximizes the output $x_5$.
...
\begin{problem}\label{prob:z3-dnn}
    Use~\autoref{ex:z3-dnn} as an example and try the following exercises:
    \begin{enumerate}
    \item Use Z3 to encode the DNN in~\autoref{fig:dnn} and compute the outputs of \emph{all} neurons for the input $(x_1, x_2) = (-1.3, 3.5)$.
    \item Use Z3 to find an input $(x_1, x_2)$ that \emph{maximizes} the output $x_5$ of the DNN in~\autoref{fig:dnn}.
    \item Add another input $x_{new}$ and update the DNN accordingly. Then, use Z3 to find an input $(x_1, x_2, x_{new})$ that {minimizes} the output $x_5$ of the updated DNN.
    \end{enumerate}
\end{problem}
...
Consider the NN in~\autoref{fig:proofnet}. Recall that in this network the hidden neurons ($v_1 \ldots v_4$) use ReLU activation, but the output neurons ($y_1,y_2$) does not.
...
\begin{problem}\label{problem:short-questions}
...
\begin{example}\label{ex:lp-example2}
Minimize
\[
z = x + y
\]
...
\begin{itemize}
    ...
    \begin{itemize}
        \item Topics: DNN and Properties and their formal representation (\autoref{chap:nn-basics} and~\autoref{chap:properties})
        \item In class exercise: Using Z3 to check satisfiability of DNNs and properties~\autoref{prob:z3-dnn}
...
\begin{example}\label{ex:milp-simple}
Maximize
\[
z = 3x + 5y
\]
...
\paragraph{Step 1: Find vertices of the feasible region (ignoring integer constraints)}
\[
\begin{aligned}
x + 2y &= 6,\\
2x + y &= 6,\\
...
Solving the line intersections:

- Intersection of \(x + 2y = 6\) and \(2x + y = 6\):
\[
x = 2, \ y = 2
\]
...
Vertices of the feasible region: \((0,0),\ (0,3),\ (3,0),\ (2,2)\).

---

\paragraph{Step 2: Evaluate objective function at vertices}
...
So the **maximum value** is \(z = 16\) at \((2,2)\).

---

\paragraph{Step 3: Integer constraint check}

All vertices are integers, so \((2,2)\) is valid for the MILP.  
