...
Here, \Deduce determines that the problem is feasible, e.g., the computed bounds of $y_1$ and $y_2$ indicate that $y_1 \le y_2$ can be satisfied. Next, \bab{} calls \mycode{LP} to check the satisfiability of the problem using an LP solver. The LP solver returns unsatisfiable (or unknown)--meaning no counterexample is found using this activation pattern.  In other words, the more precise LP check cannot find a counterexample, even though the abstraction-based \Deduce indicates that one may exist.  This is a common case because abstraction is an over-approximation and thus may yield false positives. However, in the first iteration, it is likely that LP returns unknown because the problem is too complex to solve without fixing any neuron status. In either case, we cannot conclude that the property is valid or invalid and thus needs to make the search smaller by \emph{splitting} a neuron (i.e., fixing its activation status).
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item Due date \alert{Monday April 28, 2026 11:59pm}
    \end{itemize}
