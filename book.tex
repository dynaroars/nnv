...
\reluplex{}~\cite{katz2017reluplex} is a classical BaB (\autoref{chap:bab}) approach for NNV. The technique extends the \emph{simplex} method~\cite{nelder1965simplex} to support the ReLU activation function (\textbf{Reluplex} = \textbf{Relu} + Sim\textbf{plex}). \reluplex{} has been succeeded by the more efficient Marabou~\cite{katz2019marabou} tool. However, the core ideas of \reluplex{} are still relevant and thus presented here as another example of BaB-based NNV techniques.
