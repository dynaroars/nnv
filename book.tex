...
\begin{itemize}
    ...
    \item Topics: 
    \begin{itemize}
    \item  Background on Logic and Satisfiability~\autoref{chap:logics}
    \end{itemize}
...
True: that means something statement is true but cannot be proved
False: we can prove a false statement 

Undecidable :  considered hardest problem
...
satisfiability problem  : determiene whether a formula (that only involves boolean variables) is satisfiable or not  :   NP-Complete, takes exponential time


Z3 SMT solver from Microsoft. 

Forall x,  ThereExists y,    y > x 
ThereExits x.  ForallY      y <= x 


For everynumber x, there is NO number y such that y > x 