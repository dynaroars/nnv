...
Neural network verification (NNV) is a rising topic that applies \emph{formal methods} (FM) to machine learning systems, particularly neural networks (NN).  This chapter explains what FM are, how verification differs from testing and debugging, and key concepts such as specifications and common FM techniques.
...
\begin{problem}[Minimum Specification]
    Give a specification for a function \code{min(l)} that takes a list of integers \code{l} and returns the minimum integer in the list.  Clearly state the precondition and postcondition for the function.
\end{problem}


\begin{problem}[Duplicate]
    Give a specification for a function \code{remove\_duplicates(l)} that takes a list of integers \code{l} and returns a list of integers \code{l2} that contains the same integers as \code{l} but with all duplicates removed.  Clearly state the precondition and postcondition for the function.
\end{problem}    


%\paragraph{What to Blame?} 
\section{FM Techniques}
...
\section{Major Classes of Formal Methods Algorithms}\label{sec:fm-techniques}
...
\begin{definition}
An FM algorithm is \textbf{sound} if every property it claims to prove is actually true. In other words, if it says \emph{``this property holds in this system,''} then the property truly holds. Conversely, an FM algorithm is \textbf{unsound} if it can claim that a false property is true.
\end{definition}


Two things to note about soundness:
\begin{itemize}
    \item While a sound FM algorithm never claims a false property is true, it may claims that a true property is false (i.e., it can give false counterexamples). Thus, an extremely conservative FM algorithm that claims every property is false is still sound. Similarly, an FM algorithm that returns ``unknown'' for every property is also sound. 
    \item It is important to distinguish between an FM algorithm and its implementation (the software tool). An FM algorithm may be sound, but its implementation---typically written by human (or AI nowadays)---may contain bugs that cause it to produce unsound results. 
\end{itemize}
...
\begin{itemize}
    \item While a complete FM algorithm can prove every true property, it may also claim that a false property is true (i.e., it can produce false proofs). Thus, an extremely reckless (and not trustworth) FM algorithm that claims every property is true is still complete. Similarly, an FM algorithm that returns ``unknown'' for every property is also complete. 
    \item As with soundness, it is important to distinguish between an FM algorithm and its implementation. An FM algorithm may be complete, but its implementation may contain bugs that cause it to produce incomplete results.
\end{itemize}
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item Formal Methods Overview (\autoref{chap:formal-methods})
    \end{itemize}
    \item Week 2 (Sept 3):
    \begin{itemize}
    ...
    \item    Background on Logic~\autoref{chap:logics} and Linear Programming~\autoref{chap:lp}
    \end{itemize}
    \item Quiz: TBD
    \end{itemize}
%     \item Week 3 (Sept 10):
